...
def main(args):
    ...
    if args.use_cached_knowledge:
        if args.finetune_from_model is not None:
            paths = args.finetune_from_model
        else:
            paths = args.save_dir+'/checkpoint_last.pt'

        old_models, _old_model_args = checkpoint_utils.load_model_ensemble(
            utils.split_paths(paths),
            task=task,
            suffix=getattr(args, "checkpoint_suffix", ""),
            strict=(args.checkpoint_shard_count == 1),
            num_shards=args.checkpoint_shard_count,
        )
        if "old_model" not in utils.global_vars():
            utils.global_vars()["old_model"] = old_models[0].cuda().eval()
...
def validate_and_save(args, trainer, task, epoch_itr, valid_subsets, end_of_epoch):
    ...
    do_validate = (
        ...
        or (
            ...
            # add by zx, only validation after updates
            and num_updates >= args.validate_after_updates
        )
