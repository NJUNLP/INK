...
def main(args, override_args=None):
    ...
    for subset in args.valid_subset.split(","):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(progress):
                ...
                features = task.forward_and_get_hidden_state_step(
                    sample, model)  # [B, T, H]
                target = sample['target']  # [B, T]
                # print('feature_size:{}'.format(features.size()))
                # print('target_size:{}'.format(target.size()))
                #if hasattr(args, 'decoder_langtok') and args.decoder_langtok:
                    # remove prefix langtag when task=translation_multi_simple_epoch
                #    features = features[:,1:,:].contiguous()  # [B, T, H]
                #    target = target[:,1:].contiguous()  # [B, T]
                ...
                if args.save_plain_text:
                    ...
                    src_tokens = src_tokens.index_select(
                        dim=0, index=non_pad_index)  # [n_count, src_len]
                ...
                if args.dstore_fp16:
                    dstore_keys[dstore_idx:reduce_size + dstore_idx] = features.detach().cpu().numpy().astype(
                        np.float16)
                    dstore_vals[dstore_idx:reduce_size +
                                dstore_idx] = target.unsqueeze(-1).cpu().numpy().astype(np.int)
                else:
                    dstore_keys[dstore_idx:reduce_size + dstore_idx] = features.detach().cpu().numpy().astype(
                        np.float32)
                    dstore_vals[dstore_idx:reduce_size +
    ...
    if args.save_plain_text:
        ...
        if args.save_full_context:
            for src_tokens, target in tqdm(zip(batch_src_tokens, batch_target)):

                src_strs = src_dict.string(src_tokens, return_list=True, extra_symbols_to_ignore=[
                    src_dict.pad()])  # [[str]]
                trg_tokens = tgt_dict.string(
                    target, return_list=True)  # [[token]]
                cur_trg_str = ""
                sent_len = 0
                ...
                for src_str, trg_token in zip(src_strs, trg_tokens):
                    ...
                    if len(trg_token) == 0:
                        for l in range(sent_len):
                            plain_text.append(
                                "src: {} trg: {}".format(src_str, cur_trg_str))

                        cur_trg_str = ""
                        sent_len = 0

            with open(args.dstore_mmap + '/full_context.txt', 'w') as f:
