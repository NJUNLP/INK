...
class KNN_Dstore(object):
    ...
    def __init__(self, args, trg_vocab_size):
        ...
        self.use_all_gpus_to_search = args.use_all_gpus_to_search
        ...
        if self.k_type == 'fix':
            ...
            self.mask_for_distance = self.generate_neighbor_mask(
                args.max_k if args.max_k is not None else args.k)
            self.reduce_k = self.mask_for_distance.size(0)

        self.mask_for_label_count = self.generate_label_count_mask(
            args.max_k if args.max_k is not None else args.k)
    ...
    def generate_neighbor_mask(self, max_k):
        ...
        # [1 - 1, 2 - 1, 4 - 1, 8 - 1, ...]
        power_index = torch.tensor(
            [pow(2, i) - 1 for i in range(0, int(math.log(self.max_k, 2)) + 1)])
    ...
    def generate_label_count_mask(self, max_k):
        ...
        mask_for_label_count = torch.triu(
            mask_for_label_count, diagonal=1).bool()
    ...
    def get_label_count_segment(self,
        ...
        expand_tgt_idx = expand_tgt_idx.masked_fill(
            self.mask_for_label_count, value=-1)

        labels_sorted, _ = expand_tgt_idx.sort(dim=-1)  # [B, S, K, K]
        labels_sorted[:, :, :, 1:] *= ((labels_sorted[:, :,
                                                      :, 1:] - labels_sorted[:, :, :, :-1]) != 0).long()
        ...
        if relative:
            retrieve_label_counts[:, :, 1:] = retrieve_label_counts[:,
                                                                    :, 1:] - retrieve_label_counts[:, :, :-1]
    ...
    def get_label_count(self, tgt_idx: torch.Tensor):
        ...
        tgt_sorted[:, :, 1:] *= ((tgt_sorted[:, :, 1:] -
                                  tgt_sorted[:, :, :-1]) != 0).long()
        retrieve_label_counts = tgt_sorted.ne(
            0).sum(-1).unsqueeze(-1)  # [B, S, 1]
    ...
    def setup_faiss(self, args):
        ...
        index = faiss.read_index(
            args.dstore_filename + '/knn_index', faiss.IO_FLAG_ONDISK_SAME_DIR)
        if self.use_gpu_to_search and not self.use_all_gpus_to_search:
        ...
        elif self.use_gpu_to_search and self.use_all_gpus_to_search:
            print('put index from cpu to all gpus')
    ...
    def get_knns(self, queries):
        ...
        knn_key = torch.from_numpy(self.keys[knns.cpu()]).cuda()  # [Batch size * Seq len, K, H]
        knn_dists = torch.sum((knn_key - queries.unsqueeze(-2).detach()) ** 2, dim=-1)   
        dists, new_index = torch.sort(knn_dists, dim=-1)
        knns = knns.gather(-1, new_index)
    ...
    def retrieve(self, queries):
        ...
        ).view(-1, queries.size(-1)))  # [Batch * seq len, K]
    ...
    def calculate_select_knn_prob(self,
        ...
        re_compute_dists = self.dist_func(
            distance, knn_index, queries, function=self.sim_func)  # [B, S, K]

        re_compute_dists = re_compute_dists.unsqueeze(-2).expand(B, S, R_K, K)

        # start = time.time()
        re_compute_dists = re_compute_dists * \
            self.mask_for_distance  # [B, S, R_K, K]
        ...
        weight_sum_knn_weight = torch.matmul(
            knn_select_prob.unsqueeze(-2), knn_weight).squeeze(-2).unsqueeze(-1)  # [B, S, K, 1]
        ...
        # start = time.time()
        scatter(src=weight_sum_knn_weight.float(),
                out=knn_tgt_prob, index=tgt_index, dim=-1)
    ...
    def calculate_knn_prob(self,
        ...
        scaled_dists = re_compute_dists / temperature
        knn_weight = torch.softmax(
            scaled_dists, dim=-1).unsqueeze(-1)  # [B, S, K, 1]
        ...
        scatter(src=knn_weight.float(), out=knn_tgt_prob,
...
if __name__ == "__main__":
    class ARGS:
        ...
        dstore_fp16 = True
        knn_temperature_type = None
        knn_k_type = 'fix'
        indexfile = '/home/zhuwh/experiments/datastore/opus-zx/medical/knn_index'
        dstore_filename = '/home/zhuwh/experiments/datastore/opus-zx/medical'
        no_load_keys = True
        probe = 32
        move_dstore_to_mem = True
        use_gpu_to_search = True
        use_all_gpus_to_search = False
        trg_vocab_size = 42024

    args = ARGS()
    knn_store = KNN_Dstore(args=args, trg_vocab_size=args.trg_vocab_size)

    query = torch.randn(4096, 1024)
    ...
    print('average time for retrieve neighbors, {} s'.format(
        knn_store.time_for_retrieve / knn_store.retrieve_count))
