...
class TranslationTask(LegacyFairseqTask):
    ...
    def forward_and_get_mt_knn_results(self, sample, model):
        model.args.only_return_mt_knn_results = True

        mt_prob, knn_prob, final_prob, last_hidden, knn_index, knn_dists, tgt_index \
            = model(src_tokens=sample['net_input']['src_tokens'],
                    src_lengths=sample['net_input']['src_lengths'],
                    prev_output_tokens=sample['net_input']['prev_output_tokens'],
                    return_all_hiddens=False,
                    features_only=False)
